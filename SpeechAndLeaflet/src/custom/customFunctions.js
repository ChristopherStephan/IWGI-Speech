    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            for (var i in obj) {
                ...
                var marker = L.marker(new L.LatLng(parseFloat(coordinates[0]), parseFloat(coordinates[1])), options = {
                    "id": obj[i].ID
                });
                marker
...
function addPointAttributes(name, des) {
    $.post(
        ...
        function (data) {
            $(function () {
                $("#dialog").dialog().html("Thank you, we've received your (point)-project!");
            });
            setTimeout(function () {
                $("#dialog").dialog("close");
                javascript: location.reload();
...
function addLineAttributes(name, des, type) {
    $.post(
        ...
        function (data) {
            $(function () {
                $("#dialog").dialog().html("Thank you, we've received your (line)-project!");
            });
            setTimeout(function () {
                $("#dialog").dialog("close");
                javascript: location.reload();
